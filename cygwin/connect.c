...
char g_serial_port[32]="dev/ttyS0";
...
void listusb_callback(char* path){
	...
	if (!strncmp(path,"/dev/sd",7)) return;
	if (!strncmp(path,"/dev/sr",7)) return;
	for(i=0;i<(sizeof avoid_list/sizeof avoid_list[0]);i++){
		if (!strcmp(path,avoid_list[i])) return;
	}
	printf("%s\n", path);
}
